// Tạo / sửa yêu cầu thanh toán (/payments/create và /payments/:id/edit) — bám sát mockup:
// Người xác nhận, NCC, Phân loại, Hạn TT, Nội dung, File, Phương thức TT, khoản chi.
// Khi có params.id → chế độ sửa: nạp sẵn dữ liệu phiếu và lưu bằng PUT.
import { get, post, put } from '../api.js';
import { esc, fmtMoney, options, toast, tryDo, spinner } from '../ui.js';
import { PAYMENT_GROUPS } from '../constants.js';

export async function renderPaymentCreate(root, params) {
  root.innerHTML = spinner();
  const editId = params?.id ? Number(params.id) : null;
  const [users, suppliers, existing] = await Promise.all([
    get('/users/options'),
    get('/suppliers'),
    editId ? get(`/payment-requests/${editId}`) : null,
  ]);

  if (existing && existing.status !== 'pending') {
    toast('Chỉ sửa được phiếu đang chờ duyệt', 'error');
    location.hash = '#/payments';
    return;
  }

  const approverIdsSet = new Set((existing?.approvers ?? []).map((a) => a.user_id));

  let itemSeq = 0;
  const itemRow = (item) => `
    <tr data-pay-item="${itemSeq++}">
      <td><input class="form-control form-control-sm" data-pi="description" placeholder="VD: Phôi hoodie đen size M" value="${esc(item?.description ?? '')}"></td>
      <td><input type="number" class="form-control form-control-sm" data-pi="qty" value="${esc(item?.qty ?? 1)}" min="0" step="any" style="width:90px"></td>
      <td><input class="form-control form-control-sm" data-pi="unit" placeholder="cái" style="width:90px" value="${esc(item?.unit ?? '')}"></td>
      <td><input type="number" class="form-control form-control-sm" data-pi="unit_price" value="${esc(item?.unit_price ?? 0)}" min="0" step="any" style="width:140px"></td>
  ...
  const itemsHtml = (existing?.items?.length ? existing.items : [null]).map((it) => itemRow(it)).join('');
  const groupSelected = (k) => (existing?.payment_group === k ? 'selected' : '');
  const currencySelected = (c) => ((existing?.currency ?? 'VND') === c ? 'selected' : '');

  root.innerHTML = `
    <div class="mx-auto" style="max-width:980px">
    <div class="sh-card">
      <h5 class="mb-4">${editId ? `Sửa yêu cầu thanh toán ${esc(existing.serial_number)}` : 'Tạo yêu cầu thanh toán'}</h5>
      <form id="pay-form">
        <div class="row g-3">
          <div class="col-md-6"><label class="form-label">Người xác nhận *</label>
            <select class="form-select" name="approvers" multiple size="4">${users.data
              .map((u) => `<option value="${u.id}" ${approverIdsSet.has(u.id) ? 'selected' : ''}>${esc(u.name)} (${esc(u.role_name)})</option>`).join('')}</select>
          ...
          <div class="col-md-6">
            <div class="mb-3"><label class="form-label">Nhà cung cấp</label>
              <select class="form-select" name="supplier_id" id="pay-supplier">${options(suppliers.data, { empty: 'Chọn nhà cung cấp', selected: existing?.supplier_id })}</select></div>
            <div><label class="form-label">Hạn thanh toán</label>
              <input type="date" class="form-control" name="due_date" value="${esc(existing?.due_date ? String(existing.due_date).slice(0, 10) : '')}"></div>
          </div>
          <div class="col-md-6"><label class="form-label">Phân loại *</label>
            <select class="form-select" name="payment_group" required>
              ...
              ${Object.entries(PAYMENT_GROUPS).map(([k, v]) => `<option value="${k}" ${groupSelected(k)}>${v}</option>`).join('')}
            </select></div>
          <div class="col-md-6"><label class="form-label">Đơn vị tiền tệ</label>
            <select class="form-select" name="currency"><option ${currencySelected('VND')}>VND</option><option ${currencySelected('USD')}>USD</option></select></div>

          <div class="col-12"><label class="form-label">Nội dung thanh toán</label>
            <textarea class="form-control" name="content" rows="3"
              placeholder="Thêm nội dung yêu cầu thanh toán">${esc(existing?.content ?? '')}</textarea></div>

          <div class="col-12"><label class="form-label">File đính kèm chính</label>
            <input class="form-control" name="file_main" placeholder="/files/invoices/hoa-don.pdf" value="${esc(existing?.file_main ?? '')}">
  ...
  const renderBank = (supplierId) => {
    const supplier = suppliers.data.find((s) => String(s.id) === String(supplierId));
  ...
  root.querySelector('#pay-supplier').onchange = (e) => renderBank(e.target.value);
  if (existing?.supplier_id) renderBank(existing.supplier_id);
  ...
  root.querySelector('#pay-form').onsubmit = (e) => {
    ...
    tryDo(async () => {
      ...
      if (editId) {
        const { serial_number } = await put(`/payment-requests/${editId}`, payload);
        toast(`Đã lưu phiếu ${serial_number}`);
      } else {
        const { serial_number } = await post('/payment-requests', payload);
